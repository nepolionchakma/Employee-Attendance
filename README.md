...
GOOGLE_ALLOWED_DOMAINS=        # Optional: extra restriction — comma-separated domains.
                               # Main filter is ALLOWED_EMAILS (below).
...
Only people whose **Gmail address** is listed in `ALLOWED_EMAILS` (in `.env`,
comma-separated) can sign in:

```
ALLOWED_EMAILS=arif@gmail.com,ovi@gmail.com
```
