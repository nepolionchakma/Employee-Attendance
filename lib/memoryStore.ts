...
const store = new Map<string, string>() // key: `${email||name}::${day}` -> `status\x00time\x00location` (null-byte delimited)
...
export async function markAttendanceInMemory(
  ...
  store.set(key, `${status}\x00${t}\x00${String(location || '').trim()}`)
  return true
}
