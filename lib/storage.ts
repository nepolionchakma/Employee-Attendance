...
export async function markAttendanceStatus(
  ...
  return useGoogle
    ? markAttendance(employeeName, employeeEmail as string | undefined, day as number, status as string, time, location)
    : markAttendanceInMemory(employeeName, employeeEmail as string | undefined, day as number, status as string, time, location)
}
