...
/** Batch update attendance cells (Office/Home/Absent) in one request. */
export async function batchUpdateAttendanceCells(tab, updates) {
  ...
  const data = []
  for (const { employeeName, day, status } of updates) {
...
/**
 * Admin: set a single cell to a status (Office/Home/Absent/empty). Works for
 * any tab and any day. Recomputes the Absent Days summary.
 */
export async function adminUpdateCell(tab, employeeName, dayLabel, status) {
  const sheets = await sheetsClient()
  const title = (tab || '').trim() || (await ensureMonthTab(sheets))
  const tabs = await listTabs(sheets)
  if (!tabs.includes(title)) throw new Error(`Sheet tab "${title}" not found`)
